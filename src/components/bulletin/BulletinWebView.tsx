    ...
    <div className="max-w-xl mx-auto bg-slate-50 min-h-screen pb-16 font-sans text-slate-900">
      ...
      <div className="p-4 space-y-4 -mt-2">
        ...
        {isSectionVisible(b.show_birthdays) && (b.birthdays || (b.birthday_celebrants_list && b.birthday_celebrants_list.length > 0)) && (() => {
          ...
          return (
            <div className="rounded-2xl p-4 bg-yellow-50/80 border border-yellow-200 shadow-2xs space-y-2.5">
              <div className="flex items-center justify-between">
                ...
                <div className="flex flex-wrap items-center gap-2 pt-0.5">
                  {celebrantsList.map((c, i) => (
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-yellow-300/80 bg-white hover:bg-yellow-50 text-xs font-bold text-yellow-950 transition-all shadow-2xs hover:shadow-xs hover:scale-[1.02] active:scale-95 cursor-pointer text-left group"
                    >
                      <span>🎂</span>
        ...
        {isSectionVisible(b.show_qr) && (
          <div className="rounded-2xl p-4 bg-slate-50 border border-slate-200 shadow-2xs space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              {b.custom_links && Array.isArray(b.custom_links) && b.custom_links.map((link, idx) => (
                link.url ? (
                  <a
                    key={idx}
                    href={link.url}
                    target="_blank"
                    rel="noreferrer"
                    className="p-2.5 rounded-xl bg-white border border-slate-200 hover:border-blue-400 text-slate-800 text-xs font-semibold flex items-center justify-between transition-colors shadow-2xs col-span-2 sm:col-span-1"
                  >
